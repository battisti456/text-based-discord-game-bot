...
def capture_text(
    killer_square: chess.Square,
    killer: chess.Piece,
    killed_square: chess.Square,
    killed: chess.Piece,
) -> str:
    return f"{get_square_and_piece(killer_square,killer)} captured {get_square_and_piece(killed_square,killed)}"
...
def get_move_text(board: chess.Board, move_uci: chess.Move | str) -> str:
    ...
    if (
        piece.piece_type == chess.PAWN  # en passant
        and capture is None  # not capturing
        and move_uci[0] != move_uci[2]
    ):  # but moving diagonally
        and_text = " en passant"
        capture_symbol = "p"
    ...
    if capture is not None:  # capturing
        moves_to = (
            f"captures {get_piece_name(capture.symbol())} {capture.unicode_symbol()} on"
        )
...
def get_game_over_text(board: chess.Board) -> str:
    ...
    if board.is_checkmate():
        ...
        return (
            "a draw, since 150 moves have taken place without a pawn move or a capture"
        )
...
def render_chess(
    ...
    if last_move_color is not None and len(board.move_stack) > 0:
        ...
        for square in (last_move.from_square, last_move.to_square):
            board_image.paste(last_move_square, square_to_xy(square), last_move_square)
    if check_color is not None and board.is_check():
        king_square = board.king(board.turn)
        if king_square is not None:
            check_square = PIL.Image.new("RGBA", square_size, check_color)
            board_image.paste(check_square, square_to_xy(king_square), check_square)
    if other_highlights is not None:
        for square in other_highlights:
            ...
            board_image.paste(image_square, square_to_xy(square), image_square)
    # endregion
    # region place pieces
    ...
    for i in range(BOARD_SIZE):
        for j in range(BOARD_SIZE):
            ...
            draw.text(
                ((i + 0.5) * square_edge, (j + 0.5) * square_edge),
                text=unicode,
                font=font,
                anchor="mm",
                fill=piece_color,
                stroke_width=p_white_outline if val.color else p_black_outline,
                stroke_fill=p_white_outline_color
                if val.color
                else p_black_outline_color,
            )
    # endregion
    ...
    left, top, right, bottom = draw.textbbox(
        (0, 0), "".join(chess.FILE_NAMES) + "".join(chess.RANK_NAMES), text_font, "ls"
    )
    ...
    ud_text_font = PIL.ImageFont.TransposedFont(
        text_font, PIL.Image.Transpose.ROTATE_180
    )
    for i in range(BOARD_SIZE):
        for j in range(4):
            vert = j % 2 == 0
            ...
            flip = (
                other_side != vert
            )  # if we were going to go through the effort of flipping labels
            xy = (
                border_width + int((i + 0.5) * square_edge),
                int(border_width / 2)
                if not other_side
                else image_size - int(border_width / 2),
            )
            if vert:
                xy = (xy[1], xy[0])
            xy = (
                xy[0] - (0 if flip else text_size / 4),
                xy[1] + (vert_offset if flip else -vert_offset - text_size / 4),
            )
            index = BOARD_SIZE - i - 1 if not white_perspective else i
            text = (
                chess.RANK_NAMES[BOARD_SIZE - index - 1]
                if vert
                else chess.FILE_NAMES[index]
            )
            draw.text(  # horizontal labels
                xy, text, text_color, text_font if flip else ud_text_font, "mm"
            )
    # endregion
